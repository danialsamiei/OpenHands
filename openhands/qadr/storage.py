...
def _apply_settings_defaults(settings: Settings | None) -> Settings | None:
    ...
    if os.getenv('FREEGPT_OPENHANDS_ENFORCE_LLM_DEFAULTS', 'true').lower() == 'true':
        managed_model = os.getenv('QADR_OPENHANDS_LLM_MODEL', '').strip()
        managed_base_url = os.getenv('QADR_OPENHANDS_EFFECTIVE_LLM_BASE_URL', '').strip()
        managed_api_key = os.getenv('QADR_OPENHANDS_EFFECTIVE_LLM_API_KEY', '').strip()
        if managed_model:
            settings.llm_model = managed_model
        if managed_base_url:
            settings.llm_base_url = managed_base_url
        settings.llm_api_key = SecretStr(managed_api_key) if managed_api_key else None
    return settings
