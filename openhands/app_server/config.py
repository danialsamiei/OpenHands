...
def config_from_env() -> AppServerConfig:
    ...
    if config.sandbox is None:
        ...
        if os.getenv('RUNTIME') == 'remote':
            ...
            if os.getenv('SANDBOX_DOCKER_NETWORK'):
                docker_sandbox_kwargs['network_name'] = os.environ[
                    'SANDBOX_DOCKER_NETWORK'
                ]
            if os.getenv('SANDBOX_WEBHOOK_BASE_URL'):
                docker_sandbox_kwargs['webhook_base_url'] = os.environ[
